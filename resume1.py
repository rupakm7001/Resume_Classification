...
model = pickle.load(open('resume_classification.pickle','rb'))
model1 = pickle.load(open('model_tf.pickle','rb'))
...
if menu_id == 'Home':
    ...
    tab1, tab2 = st.tabs(["💾 About Project","🦸‍♂️ About Us"])
    ...
    with tab1:
        ...
        # with col1:
            ...
            st.image("https://cdn-blog.novoresume.com/articles/cover-letter-examples/bg-mid.png")
...
### Introduction:\n\
\
#### In this project we dive into building a Machine learning model for Resume Classification using Python and basic Natural language processing techniques. We would be using Python's libraries to implement various NLP (natural language processing) techniques like tokenization, lemmatization, etc.\n\
\
...
#### The aim of this project is achieved by performing the various data analytical methods and using the Machine Learning models and Natural Language Processing which will help in classifying the categories of the resume and building the Resume Classification Model.")
    ...
    with tab2:
        ...
        # with col1:
            ...
            st.header('Sourav Parui')
            st.header('Rupak Mondal')
            st.header('Pushpendra Raj')
            st.header('Balaji Pamidi')
            st.header('Manoj J')
            st.header('Abhishek Chinchole')
            st.header('Lokesh S')
            st.title("We have used Kneighbors Classifier Model to predict the Categories of the Resume's")
    ...
    with tab1:
        ...
        if upload_file1 is not None:
            ...
            if predicted == 3:
                ...
                predicted = "SQL Developer"
            elif predicted == 1:
                predicted = "React Developer"
            elif predicted == 0:
                predicted = "PeopleSoft"

            
            st.header("The "+ upload_file1.name +" Applied for"+ " " + predicted + " " + "Profile")
    ...
    with tab2:
        ...
        for doc_file in upload_file2:
            if doc_file is not None:
                ...
                if (model.predict(model1.transform([cleaned]))) == 0:
                    predicted.append("PeopleSoft")
                elif (model.predict(model1.transform([cleaned]))) == 1:
                    predicted.append("React Developer")
                elif (model.predict(model1.transform([cleaned]))) == 2:
                    predicted.append("SQL Developer")
                elif (model.predict(model1.transform([cleaned]))) == 3:
